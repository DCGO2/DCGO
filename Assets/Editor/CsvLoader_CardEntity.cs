    ...
    IEnumerator SetCsvDataToScriptableObject(LoadCSV_CardEntity loadCSV)
    {
        ...
        for (int i = 1; i < afterParse.Length; i++)
        {
            ...
            IEnumerator CreateCardData(string CardImageName)
            {
                ...
                if (asset == null)
                {
                    // ï€ë∂êÊÇÃÉpÉXÇ…ÉtÉ@ÉCÉãÇ™ñ≥ÇØÇÍÇŒêVãKçÏê¨
                ...
                Debug.Log($"{i}/{afterParse.Length - 1}:{cardImageName}ÇÃÉfÅ[É^çÏê¨Ç™äÆóπÇµÇ‹ÇµÇΩÅB");
            }
        }
...
/// <summary>
/// string å^ÇÃägí£ÉÅÉ\ÉbÉhÇä«óùÇ∑ÇÈÉNÉâÉX
/// </summary>
