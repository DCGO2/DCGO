...
{
    public class Wanyamon_BT18_003 : CEntity_Effect
    {
                ...
                if (timing == EffectTiming.OnAllyAttack)
                {
                    ...
                    bool CanActivateCondition(Hashtable hashtable)
                    {
                        if (CardEffectCommons.IsExistOnBattleArea(card))
                        {
                            ...
                            {
                                if (card.Owner.GetBattleAreaPermanents().Count(
                                        (permanent) => permanent.TopCard.CardColors.Contains(CardColor.Yellow) && permanent.IsTamer) >= 1)
                                    return true;
                            }
                        }
