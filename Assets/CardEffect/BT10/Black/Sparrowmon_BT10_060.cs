...
using System;
using Photon.Pun;
public class Sparrowmon_BT10_060 : CEntity_Effect
{
    public override List<ICardEffect> CardEffects(EffectTiming timing, CardSource card)
    {
        ...
        if (timing == EffectTiming.None)
        {
            bool Condition()
            {
                ...
                {
                    if (CardEffectCommons.HasMatchConditionOwnersPermanent(card, (permanent) => (permanent.IsDigimon || permanent.IsTamer) && (permanent.TopCard.CardTraits.Contains("Xros Heart") || permanent.TopCard.CardTraits.Contains("XrosHeart") || permanent.TopCard.CardTraits.Contains("Twilight"))))
                    {
                        return true;
                    }
                }
